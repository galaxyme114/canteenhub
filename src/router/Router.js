// ** React Imports
import {
  Suspense, useContext, lazy, useEffect,
} from 'react';
...
const Router = () => {
  ...
  const FinalRoute = (props) => {
    ...
    // TODO: check if session has expired - if so, use Refresh token. Otherwise logout
    ...
    <AppRouter basename={process.env.REACT_APP_BASENAME}>
      <Switch>
        ...
        <Route
          exact
          path="/"
          render={() => (isUserLoggedIn() ? <Redirect to={DefaultRoute} /> : <Redirect to="/login" />)}
        />
        {/* Not Auth Route */}
        <Route
          exact
